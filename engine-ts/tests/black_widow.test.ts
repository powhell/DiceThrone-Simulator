...
describe('BW terminal states', () => {
  it('Grapple 4C upgrades=0: 6 + 0 + 2 agility = 8.0', () => {
    ...
    expect(r.currentEv).toBeCloseTo(8.0, 2)
  })

  it("Widow's Gauntlets 3B 2A upgrades=0: 6 + 0 + 1.5 CP = 7.5", () => {
  ...
  it('Vengeance large straight upgrades=0 tb=0 — rider + agility ≈ 13.44', () => {
    ...
    // 7 + riderDmg(2.0) + tbEV(2.2469) + covertOpsEV(0.1979) + 2 agility = 13.4448
    const r = BWEngine.calculateOptimalKeep([1, 2, 3, 4, 5], 0, { upgrades: 0, tbOnOpp: 0 })
    expect(r.currentEv).toBeCloseTo(13.4448, 2)
  })
  ...
  it('TB value scales at upgrades≥6 (3.36 instead of 2.8)', () => {
    // [6,6,6,6,6] upgrades=6 tb=0: Bite's OWN board value = 10 + 3.36 + 1 RRT = 14.36.
    ...
    expect(bite?.value).toBeCloseTo(14.36, 2)
  })

  it('Grapple beats Widow\'s Bite at upgrades=6 thanks to the corrected dmg + CP gain', () => {
    // [6,6,6,6,6] upgrades=6: Grapple = 6 + 6 + 2 agility + 1.5 CP + 1 RRT = 16.5
    const r = BWEngine.calculateOptimalKeep([6, 6, 6, 6, 6], 0, { upgrades: 6, tbOnOpp: 0 })
    expect(r.currentEv).toBeCloseTo(16.5, 2)
  })
})
